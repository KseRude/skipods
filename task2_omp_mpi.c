...
void FillXZBoundary(double uNew[Nx + 2][Ny + 2][N + 2]) {
    if (rowRank == 0) {
#pragma omp parallel for num_threads(4)
        for (size_t j = 0; j <= Ny; j++)
            for (size_t k = 0; k <= N; k++)
...
void FillYBoundary(size_t step, double uNew[Nx + 2][Ny + 2][N + 2]) {
#pragma omp parallel for num_threads(4)
    for (size_t i = 1; i <= Nx; i++)
        for (size_t k = 1; k <= N; k++)
        {
            if (xStart + i == N)
                continue;
            uNew[i][0][k] = Solution((xStart + i) * hx, 0, k * hz, step * tau);
            uNew[i][Ny][k] = Solution((xStart + i) * hx, 0, k * hz, step * tau);
        }
}

void InitFirstSteps(double uPPrev[Nx + 2][Ny + 2][N + 2], double uPrev[Nx + 2][Ny + 2][N + 2]) {
    {
        FillXZBoundary(uPPrev);
        FillYBoundary(0, uPPrev);
...
#pragma omp parallel for num_threads(4)
    ...
    {
        FillXZBoundary(uPrev);
        FillYBoundary(1, uPrev);
...
#pragma omp parallel for num_threads(4)
            for (size_t j = 1; j <= Ny; j++)
                for (size_t k = 1; k < N; k++)
                {
                    if (xStart + i == N || yStart + j == N)
                        continue;
                    uPrev[i][j][k] = uPPrev[i][j][k] + 0.5 * a_2 * tau * tau * (
...
#pragma omp parallel for 
        for (size_t j = 1; j <= Ny; j++)
        {
            for (size_t k = 1; k <= N; k++)
            {
                uPrev[i][Ny + 1][k] = uPrev[i][1][k];
                if (xStart + i == N || yStart + j == N)
                    continue;
                uNew[i][j][k] = 2 * uPrev[i][j][k] - uPPrev[i][j][k] + a_2 * tau * tau * (
